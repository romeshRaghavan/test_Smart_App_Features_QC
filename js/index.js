...
function setPerUnitDetails(transaction, results){
	...
	if(results!=null){
 	...
 	function checkPerUnitExceptionStatusForBEAtLineLevelForOCR(){
 		exceptionStatus="N";
 		exceptionMessage='';
...
/*	var e = { "data" : {"address": "paytm", "body":"You have made payment of Rs.135.00 to om rest.", "date_sent":1482401219880}}
 ...
 function getFormattedDateFromMillisec(input){
    ...
    return months [(themonth-1)]+"/"+thetoday+"/"+theyear;
}
...
function parseIncomingSMSForAmount(input){
    ...
    //alert("amount in index.js : "+amount);
}catch(e){
	alert("Exception index.js : " +e);
}
            	...
            	if(Array.isArray(data)){
        			...
        			for(var i in data) {
       				...
       				console.log("ADDRESS : "+sms.address + "\n Body : " + sms.body + "\n Date :"
                        ...
                        console.log("date : "+ getFormattedDateFromMillisec(sms.date));
        			}
...
function setPerUnitDetailsForOCR(transaction, results){
...
function validateExpenseDetailsForOCRandBusExp(accHeadId,expNameId,ocrFromLoc,ocrExpDate,ocrToLoc,ocrnarration,ocrUnit,ocrAmount,currencyId){
	if(ocrExpDate  == ""){
	...
	// 	}
	// }	
	if(isZero(ocrAmount,"Amount")==false){
		document.getElementById("ocrAmount").value = "";
		return false;
	}
	if(perUnitDetailsJSON.expenseIsfromAndToReqd!='N'){
	...
	if(perUnitDetailsJSON.expIsUnitReq == 'Y'){
		...
		if(ocrUnit  != ""){
			if(isOnlyNumeric(ocrUnit,"Unit")==false)
			{
				return false;
			}
...
function attachGoogleSearchBoxStaticOCR(component){
	//alert("attachGoogleSearchBox")
	//alert("component   "+component.id)
	var searchBox = new google.maps.places.SearchBox(component);
	searchBox.addListener("places_changed", function(){
