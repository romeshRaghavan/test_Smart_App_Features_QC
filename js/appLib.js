...
if (window.openDatabase) {
	...
	mydb.transaction(function (t) {
        ...
        t.executeSql("CREATE TABLE IF NOT EXISTS ocrExpenseDetails (ocrExpId INTEGER PRIMARY KEY ASC, accHeadId INTEGER REFERENCES accountHeadMst(accHeadId), expNameId INTEGER REFERENCES expNameMst(expNameId),ocrExpDate DATE, ocrFromLoc TEXT, ocrToLoc TEXT, ocrNarration TEXT, ocrUnit INTEGER, ocrAmount Double, currencyId INTEGER REFERENCES currencyMst(currencyId),isEntitlementExceeded TEXT,busExpAttachment BLOB,wayPointunitValue TEXT)");
		t.executeSql("CREATE TABLE IF NOT EXISTS BusinessExpDetailsForSMS (busExpId INTEGER PRIMARY KEY ASC, accHeadId INTEGER REFERENCES accountHeadMst(accHeadId), expNameId INTEGER REFERENCES expNameMst(expNameId),expDate DATE, expFromLoc TEXT, expToLoc TEXT, expNarration TEXT, expUnit INTEGER, expAmt Double, currencyId INTEGER REFERENCES currencyMst(currencyId),isEntitlementExceeded TEXT,busExpAttachment BLOB,wayPointunitValue TEXT)");	
    ...
    try {
		j.ajax({
            ...
            mydb.transaction(function (t) {
			  ...
			  error:function(data) {
                console.log('Error: Oops something is wrong, Please Contact System Administer'+JSON.stringify(data));
			  }
		...
		j.ajax({
		  ...
		  success: function(data) {
			  ...
			  error:function(data) {
                console.log('Error: Oops something is wrong, Please Contact System Administer');
			  }
				});	
...
function getPerUnitFromDB(expenseNameID){
   ...
   if(mydb) {
      //Get all the employeeDetails from the database with a select statement, set outputEmployeeDetails as the callback function for the executeSql command
        mydb.transaction(function (t) {
         t.executeSql("SELECT * FROM expNameMst where id="+expenseNameID, [], setPerUnitDetailsForStaticOCR);
...
function saveSMS(sms){
	...
	try{
		if(mydb) {
//alert("in saveSMS  app lib")
//alert("sms   : "+sms);
 mydb.transaction(function (t) {
        //alert("acc head start :")
        ...
        t.executeSql("SELECT * FROM accountHeadMst", [], function getDefaulAccountHead(transaction, resultsAccHead) {
            console.log("in accountHeadMst"+resultsAccHead.rows.length);

            var kk;
            
             for (kk = 0; kk < resultsAccHead.rows.length; kk++) {
                var row = resultsAccHead.rows.item(0);
                  accHead = row.accountHeadId;
             }

        t.executeSql("SELECT * FROM expNameMst where accHeadId="+accHead , [], function getDefaultExpenseName(transaction, results) {
           
           var kk;
           
              for (kk = 0; kk < results.rows.length; kk++) {
                var row = results.rows.item(0);
                  expId = row.id;
              }

        t.executeSql("SELECT * FROM currencyMst ", [], function getDefaultCurrncyName(transaction, results) {
           ...
           var kk;
              ...
              console.log("accHead : "+accHead);

                console.log("expId : "+expId);

        var accHeadId = accHead;      
        var expNameId = expId;    
      var expNarration = sms.body;
      var expFromLoc = "";     
        var expToLoc = "";   
        var expUnit =  "";    
    ...
    var expDate = getFormattedDateFromMillisec(parseInt(sms.date_sent));
    var expAmt = parseIncomingSMSForAmount(sms.body);
    ...
    if(expAmt != null || expAmt != ''){
              t.executeSql("INSERT INTO BusinessExpDetailsForSMS (accHeadId,expNameId,expDate,expFromLoc, expToLoc, expNarration, expUnit, expAmt, currencyId, isEntitlementExceeded, busExpAttachment, wayPointunitValue) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", 
                         [accHeadId,expNameId,expDate,expFromLoc,expToLoc,expNarration,expUnit,expAmt,currencyId,isEntitlementExceeded,'null','null']);
        }
        ...
        alert('Database not found, your browser does not support web sql!');
		 ...
		 //checkSMSInDatabase(expNarration);

    	}catch(e){
		console.log("Exception in app lib saveSMS : "+e);
	}
		
} 


function setPerUnitDetailsForStaticOCR(transaction, results){
   ...
   if(results!=null){
        ...
        document.getElementById("expNameMstId").value = row.expNameMstId; 

        //console.log(" expNameMstId : "+row.expNameMstId);

          //alert(">>>>>>>>>>>>>"+document.getElementById("ratePerUnit").value)
          //document.getElementById("ocrAmount").value="";
          document.getElementById("ocrUnit").value="1";
         document.getElementById("ocrFromLoc").value = "";
         document.getElementById("ocrToLoc").value = "";
         document.getElementById("ocrnarration").value = " ";
         document.getElementById("ocrUnit").value = "1";
         //document.getElementById("ocrAmount").value = "";
          if(perUnitDetailsJSON.expenseIsfromAndToReqd=='N'){
            document.getElementById("ocrFromLoc").value="";
            document.getElementById("ocrToLoc").value="";
            document.getElementById("ocrFromLoc").disabled =true;
            document.getElementById("ocrToLoc").disabled =true;
            document.getElementById("ocrFromLoc").style.backgroundColor='#d1d1d1'; 
            document.getElementById("ocrToLoc").style.backgroundColor='#d1d1d1';
            document.getElementById("ocrnarration").disabled =false;
            document.getElementById("ocrnarration").style.backgroundColor='#FFFFFF';
            //document.getElementById("mapImage").style.display= "none";
         }else{
            document.getElementById("ocrFromLoc").disabled =false;
            document.getElementById("ocrToLoc").disabled =false;
            document.getElementById("ocrFromLoc").value="";
            document.getElementById("ocrToLoc").value="";
            document.getElementById("ocrnarration").value="";
            document.getElementById("ocrFromLoc").style.backgroundColor='#FFFFFF'; 
            document.getElementById("ocrToLoc").style.backgroundColor='#FFFFFF'; 
            //alert(window.localStorage.getItem("MobileMapRole"))
            if(window.localStorage.getItem("MobileMapRole") == 'true') 
            {
               attachGoogleSearchBoxStaticOCR(document.getElementById("ocrFromLoc"));
               attachGoogleSearchBoxStaticOCR(document.getElementById("ocrToLoc"));
               //document.getElementById("mapImage").style.display="";
                document.getElementById("ocrnarration").value=" ";
               document.getElementById("ocrnarration").disabled =true;
               document.getElementById("ocrnarration").style.backgroundColor='#d1d1d1';
            } 
         }
         if(perUnitDetailsJSON.isUnitReqd=='Y'){
            ...
            if(perUnitDetailsJSON.expFixedOrVariable=='V'){
               ...
               if(perUnitDetailsJSON.expenseIsfromAndToReqd=='Y' && window.localStorage.getItem("MobileMapRole") == 'true'){
                  document.getElementById("ocrUnit").disabled =true;
                  document.getElementById("ocrUnit").style.backgroundColor='#d1d1d1';
               }
               else{
                  document.getElementById("ocrUnit").disabled =false;
                  document.getElementById("ocrUnit").style.backgroundColor='#FFFFFF';
               }
               document.getElementById("ocrAmount").disabled =false;
               document.getElementById("ocrAmount").style.backgroundColor='#FFFFFF';
            }else{
               flagForUnitEnable = true;
               if(perUnitDetailsJSON.expenseIsfromAndToReqd=='Y' && window.localStorage.getItem("MobileMapRole") == 'true'){
                  document.getElementById("ocrUnit").disabled =true;
